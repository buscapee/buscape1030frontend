...
const FormReqdemi = ({ requerimentSelected }) => {
    ...
    const { createRequerimentRelegation, editRequerimentData } = useContext(RequirementsContext);
    ...
    useEffect(() => {
        // Só atualiza automaticamente se NÃO estiver visualizando/editar um requerimento existente
        if (!requerimentSelected && user && user.users && user.users[0]) {
    ...
    return (
        <div className="flex flex-col items-center w-full">
            <form onSubmit={isEditing ? handleSaveEdit : handleSubmit} className="bg-white rounded shadow border border-gray-200 w-full max-w-3xl mx-auto flex flex-col gap-6 mt-6" style={{padding: 0}}>
                <div className="rounded-t bg-[#14532d] px-6 py-3 text-white font-bold text-lg border-b border-green-900">Efetuar Rebaixamento</div>
                <div className="flex flex-col gap-4 px-6 py-6">
                    ...
                    <div className="flex flex-col gap-2">
                        <label className="font-semibold">Rebaixado:</label>
                        <input type="text"
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <input type="text"
                            className="border rounded px-3 py-2 bg-white"
                            required
                            value={!requerimentSelected ? newPatent : requerimentSelected.newPatent}
                            disabled
                ...
                {requerimentSelected && requerimentSelected.status === "Pendente" &&
                    <section className='flex row items-center justify-center'>
                        ...
                        <button 
                        type="button"
                        onClick={handleDeleteClick} className='flex m-2 items-center justify-center text-white bg-red-700 hover:bg-red-800 text-[14px] h-[30px] w-[120px] rounded-sm font-medium'>
            ...
            <ConfirmationModal
                open={showConfirm}
                title="Confirmação"
                message={`Tem certeza que deseja excluir o requerimento de rebaixamento para \"${promoted}\"? Essa ação não poderá ser desfeita.`}
