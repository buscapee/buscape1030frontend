...
function calcularTempoTotal(inicio, fim) {
  ...
  if (segundos < 60) return `Total: ${segundos} segundos`;
...
function formatarTempoHorasMinutos(minutos) {
  const horas = Math.floor(minutos / 60);
  const min = minutos % 60;
  return `${horas} horas${min > 0 ? ` ${min} minutos` : ''}`;
}

const TodosTurnosPage = () => {
  ...
  async function fetchTurnos(params = {}) {
    ...
    try {
      ...
      if (params.nickname) query.append('nickname', params.nickname);
      if (params.dataDe) query.append('dataDe', params.dataDe);
      if (params.dataAte) query.append('dataAte', params.dataAte);
      if (params.setor) query.append('setor', params.setor);
      const res = await axiosInstance.get(`/servicepoints/all?${query.toString()}`);
      setTurnos(res.data);
      // Buscar relatório se houver nickname
      if (params.nickname) {
        const relRes = await axiosInstance.get(`/servicepoints/relatorio?${query.toString()}`);
  ...
  const handlePesquisar = (e) => {
    e.preventDefault();
    fetchTurnos({ nickname, dataDe, dataAte, setor });
    setPagina(1);
  };
  ...
  return (
    <div style={{ width: '100%', minHeight: '100vh', background: '#f5f5f5', padding: '32px 0', paddingTop: 100 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', margin: '0 32px 18px 32px' }}>
        <h2 style={{ fontWeight: 700, fontSize: 22, margin: 0 }}>Todos os Turnos</h2>
        <div style={{ display: 'flex', gap: 8 }}>
          <button style={{ background: '#e0e0e0', color: '#222', border: 'none', borderRadius: 4, padding: '7px 18px', fontWeight: 500, cursor: 'pointer' }} onClick={() => navigate('/meus-turnos')}>
            Meus Turnos
          </button>
      ...
      {/* Filtros */}
      <form onSubmit={handlePesquisar} style={{ background: 'none', margin: '0 32px 18px 32px', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <div style={{ display: 'flex', gap: 24 }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label style={{ fontWeight: 500, fontSize: 14, color: '#444' }}>Nickname</label>
            <input type="text" placeholder="Pesquisar Nickname" value={nickname} onChange={e => setNickname(e.target.value)} style={{ padding: '8px 12px', borderRadius: 4, border: '1px solid #ccc', fontSize: 15 }} />
          </div>
        </div>
        <div style={{ display: 'flex', gap: 24 }}>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label style={{ fontWeight: 500, fontSize: 14, color: '#444' }}>De</label>
            <input type="date" value={dataDe} onChange={e => setDataDe(e.target.value)} style={{ padding: '8px 12px', borderRadius: 4, border: '1px solid #ccc', fontSize: 15 }} />
          </div>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label style={{ fontWeight: 500, fontSize: 14, color: '#444' }}>Até</label>
            <input type="date" value={dataAte} onChange={e => setDataAte(e.target.value)} style={{ padding: '8px 12px', borderRadius: 4, border: '1px solid #ccc', fontSize: 15 }} />
          </div>
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <label style={{ fontWeight: 500, fontSize: 14, color: '#444' }}>Setor</label>
            <select value={setor} onChange={e => setSetor(e.target.value)} style={{ padding: '8px 12px', borderRadius: 4, border: '1px solid #ccc', fontSize: 15 }}>
              <option value="">Selecione um ou mais setores</option>
              {SETORES.map(s => <option key={s} value={s}>{s}</option>)}
            </select>
          </div>
        </div>
        <div style={{ display: 'flex', gap: 0, marginTop: 8 }}>
          <button type="submit" style={{ flex: 1, background: '#1976d2', color: '#fff', border: 'none', borderRadius: '4px 0 0 4px', padding: '10px 0', fontWeight: 600, fontSize: 15, cursor: 'pointer' }}>Pesquisar</button>
          <button type="button" onClick={handleLimpar} style={{ flex: 1, background: '#e0e0e0', color: '#222', border: 'none', borderRadius: '0 4px 4px 0', padding: '10px 0', fontWeight: 600, fontSize: 15, cursor: 'pointer' }}>Limpar</button>
        </div>
      </form>
      {/* Relatório do usuário */}
      {relatorio && (
        <div style={{ background: '#f5f5f5', borderRadius: 6, margin: '0 32px 18px 32px', padding: 0, border: '1px solid #e0e0e0' }}>
          <div style={{ fontWeight: 600, fontSize: 18, padding: '18px 18px 8px 18px' }}>Relatório de {nickname}</div>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 15, background: '#fff', borderRadius: 6 }}>
            <thead>
              <tr style={{ background: '#f7f7f7' }}>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700, width: 220 }}>Setor</th>
                <th style={{ padding: '10px 12px', textAlign: 'left', fontWeight: 700 }}>Tempo</th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(relatorio.setores || {}).map(([setor, min]) => (
                <tr key={setor}>
                  <td style={{ padding: '10px 12px' }}>{setor}</td>
                  <td style={{ padding: '10px 12px' }}>{formatarTempoHorasMinutos(min)}</td>
                </tr>
              ))}
              <tr style={{ background: '#f7f7f7', fontWeight: 700 }}>
                <td style={{ padding: '10px 12px' }}>Total</td>
                <td style={{ padding: '10px 12px' }}>{formatarTempoHorasMinutos(relatorio.total)}</td>
              </tr>
            </tbody>
          </table>
        </div>
            ...
            {loading ? (
              ...
              turnosPaginados.map((turno, idx) => (
                <tr key={turno._id} style={{ borderBottom: '1px solid #ececec' }}>
                  ...
                  <td style={{ padding: '10px 12px', textAlign: 'left', width: 70 }}>{turno.setores?.length || 1}</td>
                  <td style={{ padding: '10px 12px' }}>
                    <button
